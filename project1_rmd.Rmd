...
print(sorted_vars)
# We will reject: Treenuts, Starchy Roots, Vegetables, Spices, Fruits


# Our New Model 
new_model_2 <- lm(Deaths~Animal_Fats+Cereals+Oilcrops+Pulses+Stimulants+`Vegetal Products`+Vegetable_Oils+Obesity, data=no_unusual_observations)
summary(new_model_2)
```
...
new_model_3 <- lm(Deaths~Animal_Fats+Cereals+Pulses+Stimulants+Obesity, data=no_unusual_observations)
...
gqtest(new_model_3, point = .5, alternative = "greater", order.by = model_vif$Obesity) #need to figure out what to order by 
```
