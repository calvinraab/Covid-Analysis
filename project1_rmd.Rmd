...
# Re-estimate the models to see without unsuaul observations
...
influencePlot(modeltest, id=list(n=3)) 
...
From the residuals vs fitted plot it can be seen that our residuals appear to spread out the greater our fitted value is. The red smoother than runs through remains within +- .05. 
```{r}
par(mfrow=c(2,2))
plot(model_vif)
...
Here we tested our model by testing our model against a quadratic. Our result is a p-value of .05871 which means we should consider higher order powers. 
```{r}
library(lmtest)

resettest(model_vif, power = 2, type = "regressor")
```
...
ncvTest(model_vif) # Reject Ho

# BP test 
bptest(model_vif) #Reject Ho

# GQ Test  (not too sure what to order by here)
gqtest(model_vif, point = .5, alternative = "greater", order.by = model_vif$Obesity) #need to figure out what to order by 
```
...
```{r}
cov1 <- hccm(model_vif, type = "hc1")
model_vif.hc1 <- coeftest(model_vif, vcov. = cov1)
library(broom)
tidy(model_vif.hc1)
...
library(caret)
model_final <- model_vif # replace this with the model from 9 once we have it
